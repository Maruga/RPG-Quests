/* ═══════════════════════════════════════════════════════════════════════════════════════
   ...
   parità d'iniziativa = azioni simultanee [da validare].
   ...
   opz = { flusso, stato:{pipsPg,numPg,pipsNip,numNip,kiPg}, pg:{attr,ki,kiMax,senmon}, nip?,
           immagine, rigioca:true|false, onFine(esito, kiPg) }
   ═══════════════════════════════════════════════════════════════════════════════════════ */
(function(){
"use strict";
    ...
    if (!dif.agito && !dif.ukemi && dif.ki > 0) {
      if (dif.pg) {
        ...
        box.innerHTML = `<h4>${att.arma.nome === "mani nude" ? "Il pugno parte" : "La lama parte"} — non hai ancora agito</h4>
          <p>Decidi <b>adesso</b>, prima del suo tiro: tieni la tua azione e incassi, oppure ci rinunci per difenderti.</p>`;
        el.appendChild(box);
        await segui(box);
        ...
        if (s !== "no") { dif.ukemi = true; dif.spendiKi = (s === "ki");
