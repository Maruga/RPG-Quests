...
## Shōtotsu 衝突 — *manuale opzionale · v2.1 (sistema a scambi)*
...
**Tutto ciò che ti aiuta ABBASSA il dado. Tutto ciò che ti ostacola lo ALZA.** Senmon, mira, preparazione → si sottraggono dalla somma. Buio, fumo, distanza, colpi multipli → si sommano. Il confronto è sempre lo stesso: totale ≤ attributo.
...
- **Niente Nami e Kiwami**: in combattimento il critico è un altro (l'**1+1**, vedi *Manovre*). Il combattimento costa **Ki** — e non muove il soroban. Gli attributi i dadi non li toccano mai: può toccarli solo una tua scelta (*Stringere i Denti*)
...
3. **Chi subisce danno perde la propria azione** in quello scambio: il colpo ti ha fermato — a meno di *Stringere i Denti* (vedi Manovre)
...
**In tanti** (2 contro 3, la rissa): tirano tutti, si agisce in ordine di iniziativa, chi ha preso danno salta la propria azione quando arriva il suo momento (salvo *Stringere i Denti*). **Il bersaglio di più attacchi difende attivamente contro UNO solo** (sceglie quale); gli altri colpi passano pieni — ridotti solo dall'**Assorbe** di coperture e protezioni, che vale sempre e contro tutti. Il numero conta: la fuga resta la risposta giusta.
...
Ogni cosa che fai in uno scambio ha una **velocità**, come le armi: iniziativa = **dado basso + velocità dell'azione**. Le armi e le protezioni hanno la loro in tabella; per tutto il resto il GM la assegna per analogia:
...
| **3–4** | Un'azione composta | estrarre dalla fondina (3), ricaricare la pistola (4), indossare il giubbotto (4), scavalcare una finestra |
| **5–6** | Un'operazione | forzare una porta chiusa, liberarsi da una presa e correre via |

**Lo 0 è solo di chi è già in posizione** — il dito, non il corpo. La reazione più fulminea che un corpo possa fare parte da 1.

**La copertura protegge da quando ci sei dietro**: l'Assorbe vale dal tuo momento d'iniziativa in poi. Se l'avversario agisce prima, il colpo ti trova ancora allo scoperto — e se ti fa danno, l'azione la perdi: dietro quel muro non ci sei arrivato.
...
> **Il confine con la Sorpresa**: sotto tiro il bersaglio è **consapevole** — vede l'arma, può scegliere: parlare, arrendersi, tentare il tuffo. Se è **ignaro o impossibilitato a reagire**, non è uno scambio: è un agguato (vedi *Sorpresa e Agguati* — niente difesa).

> **Esempio — l'arresto**: il poliziotto tiene la pistola puntata (sotto tiro, iniziativa pura); il malvivente, a due passi, valuta il tuffo dietro il bancone. Il tuffo è velocità 1: dado basso +1 contro dado basso +0 — quasi sempre il grilletto arriva prima, e a quella distanza il colpo non ha malus. Se fa danno, il malvivente **perde l'azione**: a terra ci finisce, ma dietro il bancone non ci arriva. Alzare le mani è la scelta sensata — ed è la scena giusta. A **lontano** cambia tutto: il colpo prende +1/+2 di distanza e +1 sul corpo che si butta (vedi *Le Zone*) — lì il tuffo è una scommessa vera.
...
- **Agire allo scoperto sotto il fuoco costa**: chi non resta al riparo prende **+1 alla somma** della propria azione — rispondere al mitragliatore col revolver si può, ma con la testa bassa
...
**Il tiro e la distanza** — la distanza ostacola, e ciò che ostacola alza la somma:

| Tiro | Somma dell'attaccante |
|---|---|
| A **contatto** | **+1/+2** — il caos della colluttazione (già in tabella) |
| A **vicino** | nessun modificatore |
| A **lontano** | **+1/+2** — salvo il **fucile di precisione**, che a lontano ci vive: nessun malus |
| Su un bersaglio che **si sta buttando** o scarta di brusco | **+1**, cumulabile |

Ancore per il GM, non un righello: valgono le tre fasce e il buonsenso.

---

## I PNG nello Scontro

I PNG attaccano e difendono con gli stessi attributi dei PG (le comparse senza scheda: attributi 5-6, il GM decide al volo). Al posto del Ki hanno una **Riserva**:
...
In combattimento l'adrenalina copre tutto: il Genkai è sospeso, gli attributi i dadi non li toccano, il soroban non si muove. Poi cala il silenzio — ed è lì che il corpo presenta il conto.

A fine scontro, oltre a valutare il Genkai (chi è a Ki ≤ 3 quando tutto è finito, crolla lì), il GM può chiedere un **tiro di pressione** (Distacco o Silenzio — la scala del Regolamento: costa solo Ki, mai attributi) a chi ha appena vissuto qualcosa di grosso: essere stato sotto una raffica, essere stato ferito, la prima sparatoria, un morto a terra. Le mani che tremano *adesso*, la sigaretta che non si accende. Un tiro solo, non uno per ogni cosa successa — e conta nel limite dei 2-3 tiri di pressione a sessione.

---

## Le Conseguenze — Giappone 1997

Un poliziotto che **estrae** l'arma lo scrive nel rapporto. Uno che **spara** apre un fascicolo dell'ispettorato. Uno che **ferisce o uccide** finisce sui giornali, e il suo caso con lui. Anche lo scontro vinto ha un prezzo: giorni di audizioni interne, l'arma ritirata in attesa di verifica, il commissario che non ti guarda. Usalo — è più GENKAI di qualsiasi sparatoria (vedi *Quadro Legale*, Manuale GM).
...
SCAMBIO: tutti tirano 2d6, ogni scambio
  ...
  CHI SUBISCE DANNO PERDE LA PROPRIA AZIONE (salvo STRINGERE I DENTI)
...
MIRARE: fermi, -1 dal prossimo scambio, max 2; salta se difendi/muovi;
      chi mira ha l'arma PRONTA (spara a iniziativa pura)
SOTTO TIRO (azione pronta): arma già sul bersaglio = iniziativa dado basso PURO
      si assume con l'azione (o la scena la dà); decade su danno o linea persa
      bersaglio consapevole difende normale; ignaro = SORPRESA
...
DISTANZA: contatto +1/+2 | lontano +1/+2 (non il fucile di precisione)
...
*GENKAI 限界 — Lo Scontro (Shōtotsu 衝突) · modulo opzionale, v2.1 — da validare al tavolo*
