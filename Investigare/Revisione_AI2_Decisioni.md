...
- **Agg. 2026-08-04 (v2.1, decisioni utente in sessione REGOLE)**: scala **velocità delle azioni** (lo 0 solo all'azione pronta; muro "mettersi" 0→1); manovre nuove **Sotto Tiro** (arma già sul bersaglio: iniziativa = dado basso puro) e **Stringere i Denti** (danno netto 1-2: paghi 1 punto dell'attributo in uso e non perdi l'azione); **Mirare** include l'arma pronta; malus **distanza** a lontano (+1/+2, non il fucile di precisione) e **+1** agendo allo scoperto sotto soppressione; **tiro di pressione post-scontro** ("il Conto"). Tutto vive **solo** in `GENKAI_Combattimento.md`: gli altri manuali sono puro rimando, anche per la morte in combattimento. **Agg. sera**: **fallimento critico 6+6** — SOLO su attacco o azione attiva, **mai in difesa** (confermato dall'utente); 1d6 sulla tabella a tre colonne (armi da fuoco / corpo a corpo / movimento e altro), 1 blando → 6 disastroso; solo colore su meccaniche esistenti (inceppata, arma a terra, lama incrinata, cadute).
...
- **28** (Reg sez. Ki): *"Il dado del Ki si ritira **all'inizio di un nuovo caso, o quando il GM lo ritiene opportuno** (una settimana che passa, una svolta di vita). Il nuovo valore **sostituisce** il vecchio, in su o in giù; i punti Ki **comprati si sommano sempre** (es.: attributo più basso 5, dadi 2 e 4 → prendi il 4, +2 comprati = Ki massimo 11). Se il nuovo massimo è inferiore al Ki attuale, il Ki attuale scende subito al nuovo massimo. Il dado con 1 si ritira **una sola volta**."* (Nota: il caso "Genkai da ricalcolo" è impossibile — Ki max mai sotto ~5.) **[SUPERATO 2026-08-04: l'1 si ritira SEMPRE, anche più volte — decisione utente, manuali allineati al wizard.]**
