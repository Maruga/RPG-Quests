...
scambio con iniziativa = il dado migliore dei 2d6 e basta · **Ki dei PNG calcolato come per i PG** (attributo più basso +
...
Wizard casi: le statistiche PNG usano già il Ki con la formula dei PG (attributo più basso + 2d6 dado migliore, tetto 12); il 2026-09-07 tolte le ultime frasi «Riserva 3/6/9» dai testi del passo Schede e dello schema. ⚠ Il foglio da tavolo del combattimento dell'avventura Bakuon (`Avventura Tanto Rumore/SCONTRO_FOGLIO_TAVOLO.docx`, generato da `genera_foglio_scontro.py`) era tutto sulle regole v2 (iniziativa col dado basso, difesa che assorbe, Riserva 3/6/9): **buttato per decisione dell'utente (2026-09-07)**, spostato con il suo script in `Avventura Tanto Rumore/storico/`; al tavolo si stampa `Combattimento/Scheda_Giocatori_Combattimento.html` (v3.1). Le schede PG portano solo l'equipaggiamento con i numeri delle armi, non la procedura dello scontro: quella sta sulla Scheda Giocatori. — **NON SERVE** (informativa)
...
`GENKAI_Combattimento.md` — la cartella serve a elaborarci sopra, le decisioni passano come sempre
dal registro. Nessuna azione richiesta: solo saperlo. — **NON SERVE** (informativa)
