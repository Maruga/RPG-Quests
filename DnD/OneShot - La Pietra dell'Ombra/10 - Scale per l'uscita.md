
## Stanza Scale — Passaggio

### Descrizione

> [!Leggere]
> La porta si apre. Scale di pietra scendono davanti a voi.
> L'aria è fredda. Ferma.
> Scendete. Sulla sinistra, una parete di inferriate arrugginite e vetro sporco. Oltre, una stanza. Non potete entrarci da qui, ma vedete qualcosa.
> Un tavolo di pietra scura. Simboli incisi sulla superficie. Un cerchio rituale. Sopra: boccette, libri impilati, un pugnale dalla lama nera. E appoggiato alla parete — un bastone. Legno scuro, intarsi d'argento. Sembra prezioso.
> Continuate a scendere.
> In fondo, un pianerottolo. Due porte si fronteggiano. Una a sinistra, una a destra.
> Di fronte a voi, contro la parete — un baule. Massiccio. Ferro e acciaio. Una serratura grossa come un pugno.
> Non sembra facile da aprire.


## Portone Rinforzato — Verso la Stanza dei Ragni

### Descrizione

> [!Leggere]
> Un portone di ferro battuto. Massiccio. Pesante. Piastre ribattute, borchie grosse come pugni.
> Nessuna maniglia visibile. Nessuna serratura. Solo metallo freddo e silenzioso.
> I cardini sono nascosti — interni, protetti. Questo portone non si smonta.
> Sul telaio, segni sbiaditi. Rune, forse. O semplici graffi del tempo.
> È stato costruito per non essere aperto.

### Statistiche

| Elemento          | Dettaglio                              |
| ----------------- | -------------------------------------- |
| **Materiale**     | Ferro battuto, piastre rinforzate      |
| **CA**            | 0 [19]                                 |
| **HP**            | 60                                     |
| **Durezza**       | 10 (ignora i primi 10 danni per colpo) |
| **Tempra Arcana** | Si                                     |
| **Serratura**     | Nessuna (meccanismo nascosto)          |
| **Cardini**       | Interni, non accessibili               |
| **Sfondare**      | Quasi impossibile, rumore assordante   |
#### Tempra arcana
Il portone è stato trattato con magia protettiva. Effetti: 
- Acido: durezza raddoppiata (20) 
- Fuoco magico: durezza raddoppiata (20) 
- Fuoco normale: nessun effetto 
- Fulmine: danni dimezzati, poi applica durezza 
- Armi magiche: non ignorano la durezza (serve +3 o superiore)

### Meccanismo di Apertura

| Posizione | Dettaglio |
|-----------|-----------|
| **Dove** | Nicchia nascosta alla base del muro, tra le due inferriate |
| **Aspetto** | Pietra leggermente sporgente, stessa tonalità del muro |
| **Azione** | Premere la pietra + spingere il portone |

### Trovare il Meccanismo

| Metodo                              | Tiro       |
| ----------------------------------- | ---------- |
| Cercare attivamente                 | 1-2 su d6  |
| Nano                                | 1-3 su d6  |
| Pergamena dalla stanza segreta (8C) | Automatico |
| Ladro (cerca trappole)              | % -30%     |

### Aprire

| Metodo | Risultato |
|--------|-----------|
| Meccanismo + spingere | Si apre |
| Sfondare | CA 0, 60 HP, rumore enorme, armi rischiano rottura |
| Scassinare | Impossibile (nessuna serratura) |
| Bussare (incantesimo) | Funziona |
| Dissolvi Magie | Non funziona (meccanico, non magico) |
| Acido | 5 fiale, 5 turni, corrode i cardini |

## Portone d'Ingresso — Scale verso l'Uscita (11)

### Descrizione

> [!Leggere]
> Un portone massiccio. Legno scuro, spesso come un braccio, rinforzato con bande di ferro annerito. I cardini sono interni — non visibili, non smontabili.
> Una serratura elaborata, ottone scurito dal tempo. Sotto, un catenaccio pesante — chiuso dall'interno.
> Questo portone è stato costruito per tenere fuori chiunque.

### Statistiche

| Elemento       | Dettaglio                                       |
| -------------- | ----------------------------------------------- |
| **Materiale**  | Quercia rinforzata con ferro                    |
| **CA**         | 2 [17]                                          |
| **HP**         | 45                                              |
| **Serratura**  | Scassinare -20%                                 |
| **Catenaccio** | Non scassinabile, solo rimovibile dall'interno. |
| **Sfondare**   | Possibile, rumore enorme                        |

### Aprire

| Metodo                | Risultato                                                                                                   |
| --------------------- | ----------------------------------------------------------------------------------------------------------- |
| Chiave                | Apre serratura, catenaccio resta, la chiave non è presente nella casa, andata persa quando il mago è morto. |
| Scassinare            | -20%, apre solo serratura                                                                                   |
| Catenaccio            | Deve essere tolto a mano (dall'interno)                                                                     |
| Sfondare              | CA 2, 45 HP, sveglia tutto il dungeon                                                                       |
| Bussare (incantesimo) | Apre serratura E catenaccio                                                                                 |

### Nota
I PG arrivano dall'interno. Catenaccio e serratura non sono un problema per uscire — basta aprirli. Il portone è un ostacolo per chi entra, non per chi esce.


## Porta Superiore - Porta alla stanza del Mago

| Stato        | Dettaglio                                |
| ------------ | ---------------------------------------- |
| Dopo enigma  | Aperta                                   |
| Dopo 1 turno | Si chiude automaticamente (click)        |
| Poi          | Porta normale, scassinabile o sfondabile |

#### Da Leggere (quando si chiude):

> [!Leggere]
> Un click alle vostre spalle. Vi girate.
> La porta in cima alle scale si è chiusa. Da sola.
> *può essere scassinata dal ladro*

## Sfondare Inferriata

| Metodo          | Possibile? | Note                      |
| --------------- | ---------- | ------------------------- |
| Armi normali    | Sì         | CA 3, 30 HP, molto rumore |
| Piede di porco  | Sì         | 3 turni di lavoro         |
| Magia (Bussare) | No         | Non è una porta           |
| Acido           | Sì         | 2 fiale, 2 turni          |
Colpire con una arma da taglio si rischia di rompere 1 su 1d6. (Spade, Ascia, Bastone)
Con mazze nessun problema (Mazza, Martello da Guerra)

Se arma magica +1, solo con 1 con 2d6
Se arma magica +2 o più nessun rischio

Il primo danno l'arma si incrina ma può essere riparata e ancora funziona, -1 danno.
il secondo danno l'arma si rompe.

### Baule Massiccio

#### Descrizione

> [!Leggere]
> Un baule enorme. Ferro battuto, rinforzato con bande d'acciaio. Angoli ribattuti. Pesante — non si sposta.
> La serratura è grossa. Complessa. Tre cilindri visibili. Non è un lucchetto da scassinatore improvvisato.


#### Metodi per Aprire

| Metodo                    | Possibile? | Difficoltà                                        |
| ------------------------- | ---------- | ------------------------------------------------- |
| **Chiave**                | Sì         | Automatico (chiave nella stanza ragni)            |
| **Scassinare**            | Sì         | -30% al tiro, richiede grimaldelli                |
| **Sfondare**              | Si         | CA 2, HP 111, come porta ma +1 a rompere armi     |
| **Bussare (incantesimo)** | Sì         | Funziona                                          |
| **Dissolvi Magie**        | No         | Non magicamente chiuso                            |
| **Acido**                 | Sì         | 3 fiale, 3 turni, rovina parte del contenuto      |
| **Leva/Piede di porco**   | No         | Cardini interni                                   |
| **Scaldare serratura**    | Sì         | Fuoco intenso + acqua fredda = 1-2 su d6 si rompe |
### Nota per il DM 
Se provano a sfondare il baule:

> [!Leggere]
> Colpisci il baule. Il metallo risuona. Un'ammaccatura. Piccola.
> Questo baule è stato costruito per resistere. Ci vorrà tempo. E forse qualche arma rotta. 

**Consiglio:** Fai capire che è possibile ma costoso. Potrebbero preferire cercare la chiave o altri metodi.


### Meccanismo Porta Rinforzata

#### Posizione:
Nicchia nascosta alla base del muro, tra due inferiate.

#### Individuare:

| Metodo                | Tiro       |
| --------------------- | ---------- |
| Cercare attivamente   | 1-2 su d6  |
| Nano                  | 1-3 su d6  |
| Con pergamena indizio | Automatico |
### Contenuto Baule (quando aperto)

| Oggetto                  | Valore/Effetto              |
| ------------------------ | --------------------------- |
| Monete d'oro antiche     | 350 mo                      |
| Monete d'argento         | 800 ma (80 mo)              |
| Gemma grezza (topazio)   | 100 mo                      |
| Anello sigillo nobile    | 75 mo                       |
| Pergamena Palla di Fuoco | 400 mo                      |
| Chiave d'argento         | Apre stanza rituale [ragni] |
| Anello Magico            | Casuale                     |
| **Totale**               | **~1005 mo**                |

## Baule — Anello Casuale

### Tiro 1d6

| Tiro | Anello |
|------|--------|
| 1 | Anello Protezione +1 |
| 2 | Anello Resistenza Fuoco |
| 3-4 | Anello Caduta Lenta + Anello del Nuoto |
| 5-6 | Anello Occhio del Mago + Anello Sostentamento |

### Descrizioni

#### Anello Protezione +1

> Un anello d'oro semplice. Nessuna decorazione. Ma quando lo indossi, ti senti... sicuro. Come un'armatura invisibile.

**Effetto:** CA +1, TS +1
**Valore:** 5000 mo

#### Anello Resistenza Fuoco

> Oro rosso, caldo al tatto. Una fiamma stilizzata incisa. Non scotta mai.

**Effetto:** Immune al fuoco normale. +2 TS vs fuoco magico, metà danni se fallito.
**Valore:** 4000 mo

#### Anello Caduta Lenta

> Argento sottile. Una piuma incisa sulla fascia. Ti senti leggero.

**Effetto:** Cadi lentamente come una piuma. Nessun danno da caduta. 3 volte al giorno.
**Valore:** 2500 mo

#### Anello del Nuoto

> Rame verdastro. Onde stilizzate sulla fascia. Odore di sale marino.

**Effetto:** Nuota 60', respira sott'acqua 1 ora al giorno.
**Valore:** 2000 mo


#### Anello Occhio del Mago

> Bronzo scuro. Un occhio inciso che sembra seguirti. Inquietante.

**Effetto:** Individuazione Magia 3 volte al giorno.
**Valore:** 2500 mo

#### Anello Sostentamento

> Ferro opaco. Una spiga di grano incisa. Semplice, antico.

**Effetto:** Non hai bisogno di cibo né acqua per tre giorni consecutivi
**Valore:** 2000 mo

### Valore per Opzione

| Tiro | Contenuto | Valore Totale |
|------|-----------|---------------|
| 1 | Protezione +1 | 5000 mo |
| 2 | Resistenza Fuoco | 4000 mo |
| 3-4 | Caduta Lenta + Nuoto | 4500 mo |
| 5-6 | Occhio + Sostentamento | 4500 mo |

## PX — Stanza Scale

### Tesoro

| Oggetto | Valore |
|---------|--------|
| Monete d'oro antiche | 350 mo |
| Monete d'argento | 80 mo |
| Gemma grezza (topazio) | 100 mo |
| Anello sigillo nobile | 75 mo |
| Pergamena Palla di Fuoco | 400 mo |
| Anello/i Magico/i | 5000 mo |
| **Totale Tesoro** | **6005 mo** |

### PX Totali

| Fonte | PX |
|-------|-----|
| Meccanismo trovato | 50 |
| Tesoro | 6005 |
| **Totale** | **6055 PX** |

Diviso per 3 PG = **~2.018 PX a testa**
Diviso per 4 PG = **~1.514 PX a testa**
Diviso per 5 PG = **~1.211 PX a testa**