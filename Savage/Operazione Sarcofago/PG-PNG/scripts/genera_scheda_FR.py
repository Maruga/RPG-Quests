...
PHOTO = os.path.normpath(os.path.join(BASE, '..', 'Immagini', 'Operatori', 'Laurent_Premiere_Francia.jpg'))
...
# Flag bar — Francia: blu / bianco / rosso
...
shd(flag.rows[0].cells[0], '002395')
shd(flag.rows[0].cells[1], 'FFFFFF')
shd(flag.rows[0].cells[2], 'ED2939')
noborder(flag)
...
r = p.add_run('OPERAZIONE PROMETHEUS — DOSSIER 04/05'); r.font.name = F_BODY; r.font.size = Pt(8); r.font.color.rgb = RGBColor(0x66,0x66,0x66)
...
r = p.add_run('"PREMIERE"'); r.bold = True; r.font.name = F_TITLE; r.font.size = Pt(32)
p = left.add_paragraph()
r = p.add_run('LAURENT MARCHETTI'); r.font.name = F_TITLE; r.font.size = Pt(15); r.font.color.rgb = RGBColor(0x44,0x44,0x44)
p = left.add_paragraph(); p.paragraph_format.space_before = Pt(4)
for lbl, val in [('NAZIONALITA\'','Francia'), ('ETA\'','44'), ('UNITA\'','2e REP — Legion Etrangere'), ('RUOLO','Assaltatore / Breacher'), ('RANGO','Eroico')]:
...
txt('26 anni Legione Straniera, 2e REP. Mali, Costa d\'Avorio, Afghanistan. Esperto CQB. Cicatrice dal sopracciglio al mento. Rumoroso, esuberante, volgare. Fischietta entrando in stanza piena di nemici. "Se dice che entra per primo, entra per primo."', size=8.5, sa=2)

# Attributi
hdr('Attributi e Derivati')
txt('Agilita\' d10 | Forza d8 | Intelligenza d6 | Spirito d8 | Vigore d8      Passo 8 | Parata 7 | Robustezza 10 (4)', bold=True, size=9, sa=2, font=F_MONO)

hdr('Abilita\'', level=2)
txt('Atletica d8 | Combattere d10 | Comuni d6 | Furtivita\' d6 | Intimidire d6 | Percezione d6 | Persuasione d4 | Riparazione d6 | Sparare d10', size=8.5, sa=2, font=F_MONO)

# Vantaggi
hdr('Vantaggi')
edge('Primo Colpo Migliorato', '+4 danno al primo attacco contro nemico che non ti ha visto')
edge('Contrattacco', 'Tiro Combattere gratuito se il nemico manca con 1 o meno')
...
edge('Frenesia', '2 attacchi Combattere per round senza penalita\'')

# Svantaggi
hdr('Svantaggi')
edge('Eroico (Maggiore)', 'Non si tira mai indietro — va sempre in prima linea. Se scappi, perdi un Benny')
edge('Leale (Minore)', 'Non tradiresti mai i tuoi amici e alleati')
edge('Peculiarita\' (Minore)', 'Humor volgare, fischietta in combattimento')
...
txt('Armatura: Giubbotto tattico +4 torso | Elmetto +4 testa — Equip: Cariche da breccia x6 | Ariete portatile | Flashbang x4 | Fumogene x2', size=8, sa=2)

# Tracker
hdr('Tracker')
tracker = doc.add_table(rows=6, cols=2)
...
data = [
    ...
    ('HK416',    '30: [ ][ ][ ] (casella=10)'),
    ('BENELLI',  '8: [ ][ ][ ][ ][ ][ ][ ][ ] (casella=1)'),
    ('GLOCK/EQ', 'Glock: [ ][ ] (8+9)  Brecce: [ ][ ][ ][ ][ ][ ]  Flash: [ ][ ][ ][ ]'),
]
...
for c in flag2.rows[0].cells:
    c.height = Cm(0.3); c.text = ''
    c.paragraphs[0].paragraph_format.space_after = Pt(0)
shd(flag2.rows[0].cells[0], '002395')
shd(flag2.rows[0].cells[1], 'FFFFFF')
shd(flag2.rows[0].cells[2], 'ED2939')
noborder(flag2)
...
hdr('Il Tuo Segreto')
txt('', sa=6)
txt('Helena Weiss e\' tua figlia.', size=12, bold=True, sa=4)
txt('22 anni. Fisica. Fa parte del team scientifico della missione.', size=11, sa=4)

txt('Durante le visite mediche pre-missione, gli esami genetici hanno confermato la parentela.', size=11, sa=4)
txt('Lei non sa chi sei.', size=11, bold=True, sa=6)

txt('Se lo scoprono, ti rimuovono dalla missione.', size=11, sa=2)
txt('E chi la protegge?', size=14, bold=True, sa=10)

hdr('Il Dilemma')
txt('', sa=4)
txt('Sei un soldato. Un legionario. Un padre che non ha mai potuto esserlo.', bold=True, size=12, sa=6)
txt('Questa ragazza non sa niente di te. Per lei sei solo un operatore rude e rumoroso.', size=11, sa=4)
txt('Ma tu sai. E non puoi non proteggerla.', size=11, sa=8)

txt('Se ti concentri su di lei, metti a rischio la missione.', size=11, sa=4)
txt('Se segui la missione, potresti non riuscire a proteggerla.', size=11, sa=10)

hdr('Come Giocare')
txt('NON rivelare a nessuno che Helena e\' tua figlia. Se lo scoprono, ti tolgono dalla missione.', size=10, bold=True, sa=4)
txt('Tieni d\'occhio Helena senza dare nell\'occhio. Trova scuse per starle vicino.', size=10, sa=4)
txt('Se qualcuno la minaccia, reagisci — ma cerca di mascherare il motivo.', size=10, sa=4)
txt('Il GM ti passera\' fogliettini con situazioni che riguardano Helena. Reagisci di conseguenza.', size=10, sa=4)
txt('Se lei scopre chi sei durante la sessione: improvvisa. Sara\' un momento potentissimo.', size=10, sa=10)

txt('CLASSIFIED // OPERAZIONE PROMETHEUS // NATO SECRET', bold=True, size=7,
    color=RGBColor(0x99,0,0), align=WD_ALIGN_PARAGRAPH.CENTER)

# SAVE
out = os.path.join(BASE, 'FR_Laurent_Premiere_Scheda.docx')
doc.save(out)
print(f'Salvata: {out}')
